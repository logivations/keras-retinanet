...
def read_image_bgr(path, set_name='test'):
    ...
    if set_name == 'test':
        ...
        return augment_brightness_camera_images(image[:, :, ::-1].copy())


def augment_brightness_camera_images(image):
    result_img = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
    result_img = np.array(result_img, dtype=np.float64)
    random_bright = .5 + np.random.uniform()
    result_img[:, :, 2] = result_img[:, :, 2] * random_bright
    result_img[:, :, 2][result_img[:, :, 2] > 255] = 255
...
def resize_image(img, min_side=800, max_side=1333): #800, 1333
